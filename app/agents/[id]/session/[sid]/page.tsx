...
export default function AgentSession() {
  ...
  // The task that produced this transcript, when there is one. Worth a second request purely for the
  // title: a session's derived title comes from its LAST user prompt, and for an assigned run that is
  // the wrap-up message, so the honest name for this page lives on the task record rather than in the
  // transcript. Falls back to the derived title for an ordinary chat, which has no task.
  ...
  useEffect(() => {
    ...
    fetch(`/api/agents/${id}/tasks`)
      .then((r) => r.json())
      .then((d) => { if (alive) setRunTitle(d.tasks?.find((t: { sessionId?: string }) => t.sessionId === sid)?.title || null); })
      .catch(() => { /* an ordinary chat — the derived title is the right one */ });
    return () => { alive = false; };
  }, [sid, id]);
    ...
    <div className="bg-bento min-h-screen text-neutral-100">
      <header className="flex items-center gap-3 px-6 py-4">
        ...
        <div className="min-w-0">
          <h1 className="truncate text-sm font-semibold tracking-tight">{runTitle || meta?.title || "Transcript"}</h1>
      ...
      <main className="mx-auto max-w-3xl space-y-4 px-6 pb-16">
        ...
        {turns?.length === 0 && <p className="text-[12px] text-neutral-600">No transcript with that id — it may have been deleted.</p>}
        {turns?.map((t, i) => (
