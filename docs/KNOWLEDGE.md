...
| [`20-blacksmith.md`](knowledge/20-blacksmith.md) | §20 | ~3,600 | The chat pane as the Blacksmith agent factory's operator console: the ⚒ mode, the read-only panel over `smith ui serve`, the tile badge. Why the factory cannot tell you it is running, why a "live agent" count only ever goes up, why this integration reads and never writes — and (§20.5b) how the mode measures that it is actually in effect from a folder that isn't the clone. |
